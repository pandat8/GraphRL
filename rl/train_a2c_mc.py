...
class TrainModel_MC:
    ...
    def train_and_validate(self, n_epochs, lr_actor, lr_critic, use_critic, gamma=0.99):
        ...
        for epoch in range(n_epochs):
            ...
            plt.title(
                'RL-MonteCarlo learning curve with pretrain trainDataset ' + self.train_dataset.__class__.__name__ + ' (average number of filled edges)')
            plt.ylabel('number of fill-in')
            # plt.draw()
            plt.savefig(
                './results/acmc_' + str(
                    lr_actor) + '_' + self.heuristic + '_curve_g2m_number_gcn_logsoftmax__pretrain_train_' + self.train_dataset.__class__.__name__ + '_cuda' + str(
                    self.use_cuda) + '_with_epsilon0_1_return_test.png')
            plt.clf()
            ...
            plt.legend(('GNN-RL-epsilon', self.heuristic),  # 'GNN-RL', 'GNN-RL-epsilon', 'min-degree'
                       loc='upper right')  # 'GNN-initial', 'GNN-RL', 'min-degree'
            plt.title(
                'RL-MonteCarlo learning curve with pretrain validationDataset' + self.train_dataset.__class__.__name__ + ' (average number of filled edges)')
            plt.ylabel('number of fill-in')
            # plt.draw()
            plt.savefig(
                './results/acmc_' + str(
                    lr_actor) + '_' + self.heuristic + '_curve_g2m_number_gcn_logsoftmax_pretrain_val_' + self.train_dataset.__class__.__name__ + '_cuda' + str(
                    self.use_cuda) + '_with_epsilon0_1_return_test.png')
            plt.clf()
