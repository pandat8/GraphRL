import torch
import numpy as np
from torch.utils.data import Dataset
from data.graph import Graph
from utils.utils import erdosrenyi


class ErgDataset(Dataset):

    def __init__(self, n_nodes_lowerb, n_nodes_upperb, p_lowerb, p_upperb, n_graphs, random_seed = 32):
        """
        Generate a graph dataset with specific graph instances
        :param n_graphs: number of graphs
        :param n_nodes:  number of nodes
        :param random_seed:
        """
        super(ErgDataset, self).__init__()
        torch.manual_seed(random_seed)

        self.dataset = []
        self.p_lowerb = p_lowerb
        self.p_upperb = p_upperb
        self.n_nodes_lowerb = n_nodes_lowerb
        self.n_nodes_upperb = n_nodes_upperb

        for l in range(n_graphs): # feed random Erdos-Renyi graph instances into dataset
            p= np.random.uniform(self.p_lowerb,self.p_upperb, 1)
            n_nodes = np.random.randint(self.n_nodes_lowerb, self.n_nodes_upperb)
            g = Graph.erdosrenyi(n=n_nodes, p=p)
            self.dataset.append(g)

        self.size = len(self.dataset)

    def __len__(self):
        return self.size

    def __name__(self):
        return str('ErgDatset')

    def __getitem__(self, idx):
        return self.dataset[idx]

class AdjacencyDataset(Dataset):

    def __init__(self, n_nodes, n_graphs, random_seed = 32):
        """
        Generate a graph dataset with specific graph instances
        :param n_graphs: number of graphs
        :param n_nodes:  number of nodes
        :param random_seed:
        """
        super(AdjacencyDataset, self).__init__()
        torch.manual_seed(random_seed)

        self.dataset = []

        for l in range(n_graphs): # feed random Erdos-Renyi graph instances into dataset
            M = erdosrenyi(n_nodes)
            self.dataset.append(M)

        self.size = len(self.dataset)

    def __len__(self):
        return self.size

    def __getitem__(self, idx):
        return self.dataset[idx]
