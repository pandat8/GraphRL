    ...
    for X in data_loader:
        ...
        for x in X:
            ...
            for i in range(x.n-1):
                ...
                q_onestep[i], q2_onestep[i] = x2.onestep_greedy()
            ...
            for j in range(samle_gcn):
                ...
                for i in range(x.n-1):
                    ...
                    features = torch.FloatTensor(features)
                    degree = torch.FloatTensor(x3.onestep_greedy_d())
                    if is_cuda:
                        M_gcn = M_gcn.cuda()
                        features = features.cuda()
                        degree = degree.cuda()
...
plt.title('histogram of 1000 text graphs by learning one-step')
plt.xlabel('loss ratio: model/heuristic')
plt.savefig('./onstep.png')

#  average ratio 1.0134 max ratio 1.2432 min ratio 0.7959
