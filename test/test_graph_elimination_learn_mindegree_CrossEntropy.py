        ...
        for x in X:
            for epoch in range(epoches):
                ...
                for i in range(n - 2):
                    ...
                    node_chosen, z = x1.min_degree(x1.M)  # get the min-degree distribution of graph as label
    ...
    for X in train_loader:
        for x in X:
            ...
            for i in range(n - 2):
                ...
                m = utils.to_sparse(m)  # convert to coo sparse tensor
                features = torch.FloatTensor(features)
                node_chosen, z = x.min_degree(x.M)  # get the min-degree distribution of graph as label
                node_chosen = torch.from_numpy(np.array(node_chosen))  # one-hot coding
                node_chosen = node_chosen.reshape(1)
                if is_cuda:
                    m = m.cuda()
    ...
    for X in data_loader:
        ...
        for x in X:
            ...
            for i in range(x.n-2):
                ...
                node_chosen, d_min = x1.min_degree(x1.M)  # get the min-degree distribution of graph as label
...
text_file = open("./results/Output_learn_mindegree_ce.txt", "w")
text_file.write('\n Test result: test_graph_elimination_learn_mindegree_CrossEntropy\n')
text_file.write('DataSet: 1UFSMDataset\n')
