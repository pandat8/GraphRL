...
parser.add_argument('--ngraph_test', type=int, default=10, help='Number of graph for test dataset')
...
def evaluate(model, data_loader, features, is_cuda=args.cuda, validation = True):
    ...
    for X in data_loader:
        for x in X:
            m = torch.FloatTensor(x.M)
            features = torch.FloatTensor(features)
            node_chosen, z = x.min_degree(x.M)
            node_chosen = torch.from_numpy(np.array(node_chosen)) # one-hot coding
            node_chosen = node_chosen.reshape(1)
            if is_cuda:
                m = m.cuda()
...
def test2(model, features, data_loader, is_cuda=args.cuda, validation = True):
    """
    Evaluation function
    :param model: network model
    :param data_loader: dataset loader depending on validation or test
...
ratio, av_ratio, max_ratio, min_ratio, ratio_g2r, av_ratio_g2r, max_ratio_g2r, min_ratio_g2r = test2(model, features, test_loader, validation=False)
print('Test result:',
          'average ratio {:.4f}'.format(av_ratio),
          'max ratio {:.4f}'.format(max_ratio),
          'min ratio {:.4f}'.format(min_ratio),
          'average ratio graph2random {:.4f}'.format(av_ratio_g2r),
          'max ratio graph2random {:.4f}'.format(max_ratio_g2r),
          'min ratio graph2random {:.4f}'.format(min_ratio_g2r)
          )

plt.hist(ratio, bins= 'auto')
plt.title('graph2mindegree_ratio__CrossEntropy')
