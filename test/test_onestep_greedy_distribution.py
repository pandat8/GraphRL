...
parser.add_argument('--ngraph_test', type=int, default=4, help='Number of graph for test dataset')
...
def train(model, opt, train_loader, features,  is_cuda = args.cuda):
    ...
    for X in train_loader:
        ...
        for x in X:
            ...
            degree = torch.FloatTensor(x.onestep_greedy_d()) # get the min-degree distribution of graph as label
            if is_cuda:
                m = m.cuda()
...
def evaluate(model, data_loader, features, is_cuda=args.cuda, validation = True):
    ...
    for X in data_loader:
        for x in X:
            m = torch.FloatTensor(x.M)
            features = torch.FloatTensor(features)
            degree = torch.FloatTensor(x.onestep_greedy_d())
            if is_cuda:
                m = m.cuda()
                features = features.cuda()
                degree = degree.cuda()
...
# Train the model
features = np.ones([args.nnode,nin], dtype=np.float32) # initialize the features for training set
...
# Test the model
epoch = 0
features = np.ones([args.nnode_test, nin], dtype=np.float32) # initialize the features for test set
