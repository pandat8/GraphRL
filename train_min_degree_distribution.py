...
from gcn.models_gcn import GCN
...
def train(model, opt, train_loader, features,  is_cuda = args.cuda):
    ...
    for X in train_loader:
        ...
        for x in X:
            ...
            degree = torch.FloatTensor(x.min_degree()) # get the min-degree distribution of graph as label
            if is_cuda:
                m = m.cuda()
...
def evaluate(model, data_loader, features, is_cuda=args.cuda, validation = True):
    ...
    for X in data_loader:
        for x in X:
            m = torch.FloatTensor(x.M)
            features = torch.FloatTensor(features)
            degree = torch.FloatTensor(x.min_degree())
            if is_cuda:
                m = m.cuda()
                features = features.cuda()
                degree = degree.cuda()
